...
export function UploadOutcome({ result }) {
  ...
  return (
    <Card title="Upload outcome">
      ...
      {result.ledger_acknowledged === false && <p className="text-sm text-amber-700">The latest outcome was not confirmed in the ledger. A lookup may return an older checkpoint.</p>}
      {!complete && <p className="text-sm text-amber-700 mt-2">Completion is not confirmed. Partial writes or artifacts may remain. Review the data before retrying; lookup does not resume this upload.</p>}
